...
zone1.arima_predict <- forecast(zone1.arima.model, h = valid, xreg=predictors_test)
zone1.arima_predict.ts <- ts(arima_predict$mean, stest.arima_predict$mean, start = c(45, 1), end= c(52,7), frequency = 7)
```

```{r}
accuracy(zone1.arima_predict, yValid1.ts)
```
...
test.zone1.arima.model <- Arima(yTrain1.ts, order = c(1,1,1), xreg = train_predictors)
